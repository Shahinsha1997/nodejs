...
var ApiTokenClient = class {
  ...
  async list() {
    ...
    return _nullishCoalesce(response.tokens, () => ( []));
  }
  async create(name) {
    const response = await TursoClient.request(
...
var OrganizationClient = class {
  ...
  async list() {
    ...
    return _nullishCoalesce(response.organizations, () => ( []));
  }
  async update(options) {
    ...
    return _nullishCoalesce(response.organization, () => ( null));
  }
  async delete() {
  ...
  async members() {
    ...
    return _nullishCoalesce(response.members, () => ( []));
  }
  ...
  async invoices() {
    ...
    return _nullishCoalesce(response.invoices, () => ( []));
  }
};
...
var GroupClient = class {
  ...
  async list() {
    ...
    return _nullishCoalesce(response.groups, () => ( []));
  }
  async get(name) {
    const response = await TursoClient.request(
  ...
  async createToken(groupName, options) {
    ...
    if (_optionalChain([options, 'optionalAccess', _ => _.expiration])) {
      queryParams.set("expiration", options.expiration);
    }
    if (_optionalChain([options, 'optionalAccess', _2 => _2.authorization])) {
    ...
    const response = await TursoClient.request(
      ...
      this.config,
      {
        ...
        body: JSON.stringify({
          permissions: {
            read_attach: {
              databases: _nullishCoalesce(_optionalChain([options, 'optionalAccess', _3 => _3.permissions, 'optionalAccess', _4 => _4.read_attach, 'optionalAccess', _5 => _5.databases]), () => ( []))
            }
          }
        })
      }
...
var DatabaseClient = class {
  ...
  async list() {
    ...
    return (_nullishCoalesce(response.databases, () => ( []))).map((db) => this.formatResponse(db));
  }
  async get(dbName) {
  ...
  async create(dbName, options) {
    ...
    if (_optionalChain([options, 'optionalAccess', _6 => _6.seed])) {
    ...
    if (_optionalChain([options, 'optionalAccess', _7 => _7.seed, 'optionalAccess', _8 => _8.timestamp])) {
  ...
  async listInstances(dbName) {
    ...
    return _nullishCoalesce(response.instances, () => ( []));
  }
  async getInstance(dbName, instanceName) {
    ...
    return _nullishCoalesce(response.instance, () => ( null));
  }
  async createToken(dbName, options) {
    const queryParams = new URLSearchParams();
    if (_optionalChain([options, 'optionalAccess', _9 => _9.expiration])) {
      queryParams.set("expiration", options.expiration);
    }
    if (_optionalChain([options, 'optionalAccess', _10 => _10.authorization])) {
      queryParams.set("authorization", options.authorization);
    }
    const response = await TursoClient.request(
      ...
      this.config,
      {
        ...
        body: JSON.stringify({
          permissions: {
            read_attach: {
              databases: _nullishCoalesce(_optionalChain([options, 'optionalAccess', _11 => _11.permissions, 'optionalAccess', _12 => _12.read_attach, 'optionalAccess', _13 => _13.databases]), () => ( []))
            }
          }
        })
      }
  ...
  async usage(dbName, options) {
    const queryParams = new URLSearchParams();
    if (_optionalChain([options, 'optionalAccess', _14 => _14.from])) {
      queryParams.set("from", this.formatDateParameter(options.from));
    }
    if (_optionalChain([options, 'optionalAccess', _15 => _15.to])) {
...
var TursoClientError = class extends Error {
  constructor(message, additionalInfo) {
    ...
    this.status = _optionalChain([additionalInfo, 'optionalAccess', _16 => _16.status]);
  }
};
